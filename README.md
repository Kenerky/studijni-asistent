...
Server má nastavenou statickou IP adresu pro izolovaný segment sítě

* **IP adresa:** `10.10.10.1`
* **Maska sítě:** `/24` (255.255.255.0)
* **Rozhraní:** *enp0s8* (nebo dle reálného rozhraní)

## 2. Služby sítě (DNSMasq)
Server zajišťuje DHCP a DNS pro klienty v izolované síti. Konfigurace je v `/etc/dnsmasq.conf`.
...
sudo ufw enable# 🎓 Studijní Asistent s AI - Dokumentace
